---
title: "웹사이트에 눈송이 효과와 8비트 뮤직 플레이어 추가하기"
date: "2024-12-22"
excerpt: "개인 웹사이트에 Canvas API로 눈송이 효과를 구현하고, Tone.js로 8비트 스타일의 배경음악 플레이어를 만드는 과정"
tags: ["Web Development", "Canvas", "Tone.js", "UX"]
---
...
### 해결하지 못한 문제: 브라우저 자동 재생 정책

결국 완벽하게 해결하지 못한 부분이 있다. 바로 브라우저의 자동 재생 정책(Autoplay Policy)이다.

현대 브라우저들은 사용자 경험을 위해 사용자 상호작용 없이 오디오를 자동 재생하는 것을 차단한다. 페이지를 이동하면 새로운 문서가 로드되면서 이전의 사용자 상호작용 컨텍스트가 사라진다. 그래서 아무리 재생 상태를 저장해도, 페이지 전환 후 자동으로 이어서 재생하는 것은 불가능했다.

대안으로 "음악을 이어서 재생할까요?" 프롬프트를 띄우는 방식을 선택했다. 사용자가 "재생" 버튼을 클릭하면 그때서야 음악이 다시 시작된다. 완벽한 해결책은 아니지만, 브라우저 정책을 준수하면서 사용자에게 선택권을 주는 절충안이다.

Service Worker를 활용한 백그라운드 오디오 재생도 검토했지만, 정적 사이트 환경에서는 복잡도 대비 효과가 크지 않아 포기했다.
...
5. **완벽하지 않아도 괜찮다** - 페이지 전환 시 자동 재생은 해결하지 못했지만, 프롬프트를 통한 대안을 제시했다. 모든 문제를 기술적으로 해결할 수 있는 건 아니다.

이제 웹사이트에 들어가면 은은하게 눈이 내리고, 8비트 음악이 흐른다. 페이지를 이동하면 끊기긴 하지만, 다시 재생할 수 있는 선택지가 있다. 완벽하진 않지만 분위기가 확 달라졌다.
