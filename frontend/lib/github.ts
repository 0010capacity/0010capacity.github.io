...
export class GitHubAnalyzer {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) {
          throw new Error(
            `GitHub API rate limit exceeded. Please provide a valid token or wait before retrying.`
          );
  ...
  private async inferTechnologies(
    repo: GitHubRepo,
    ...
    if (language === "Python") {
      ...
      if (
        repoName.includes("tensorflow") ||
        description.includes("tensorflow")
      ) {
      ...
      if (
        description.includes("reinforcement learning") ||
    ...
    if (
      language === "Kotlin" ||
      (language === "Java" &&
        (repoName.includes("android") || description.includes("android")))
    ) {
  ...
  getFormattedTechStack(result: TechStackResult): string[] {
    ...
    const uniqueTechnologies = result.technologies.filter(
      tech =>
        !techStack.some(
          existing => existing.toLowerCase() === tech.toLowerCase()
        )
    );
...
export const analyzeUserTechStack = async (
  username: string,
  token?: string
): Promise<string[]> => {
  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.message.includes("403")) {
        throw new Error(
          "GitHub API 접근이 거부되었습니다. 토큰 권한을 확인해주세요. (repo 권한 필요)"
        );
      } else if (error.message.includes("422")) {
        throw new Error(
          "파일 업데이트 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요."
        );
      } else if (error.message.includes("404")) {
        throw new Error(
          "레포지토리를 찾을 수 없습니다. 레포지토리 권한을 확인해주세요."
        );
      } else if (error.message.includes("401")) {
        throw new Error(
          "GitHub 토큰이 유효하지 않습니다. 토큰을 다시 확인해주세요."
        );
      }
    }

    throw new Error(
      "Failed to create PR. Please check your token and try again."
    );
  }
};
